...
function deploy(target, mrskCommands, options) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (!target) {
      ...
      args = ['-d', target].concat(mrskCommands);
    }
    const runner = new toolrunner.ToolRunner('mrsk', args, options);
    yield runner.exec();
  });
}

function installMrsk() {
  return __awaiter(this, void 0, void 0, function* () {
    const runner = new toolrunner.ToolRunner("alias", "mrsk='docker run --rm -it -v $HOME/.ssh:/root/.ssh -v /var/run/docker.sock:/var/run/docker.sock -v ${PWD}/:/workdir  ghcr.io/mrsked/mrsk'");
    yield runner.exec();    
  });
}
...
function run() {
  return __awaiter(this, void 0, void 0, function* () {
    const mrskCommands = JSON.parse(core.getInput('mrsk_commands'));
